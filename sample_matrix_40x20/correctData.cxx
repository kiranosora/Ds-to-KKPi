...
void correctData(int recal=0){
    ...
    TH1F *m12H=new TH1F("m12H","m12H",NbinM, mBeg, mEnd);
    ...
    TFile *fMD;
    double *eff0Array=new double[NbinM*NbinCos];
    double *effArray=new double[NbinM*NbinCos];
    double *eff0Array_Inverse=new double[NbinM*NbinCos];
    ...
    for(int iN=0; iN < NbinM*NbinCos; iN++){
    ...
    iEff.close();
    double *xm12 = new double[NbinM];
    double *x12 = new double[NbinM];
    double *cos12 = new double[NbinCos];
    TH1F *m12eff_0=new TH1F("m12eff_0","m12eff_0",NbinM, mBeg, mEnd);
    for(int binIdx=0; binIdx < NbinM; binIdx++){
    ...
    for(int binIdx=0; binIdx < NbinCos; binIdx++){
        cos12[binIdx] = cosBeg + (binIdx+0.5)*binCos;
    }
    fR->cd();
    gStyle->SetMarkerStyle(2.0);
    gStyle->SetTitleSize(0.045);
    TGraph2D *gr0 =new TGraph2D(NbinM*NbinCos, xM13, yM12, eff0Array);
    gr0->SetMarkerColor(kRed);
    gr0->Write("eff0");
    TGraph2D *grNSel =new TGraph2D(NbinM*NbinCos, xM13, yM12, Ni);
    grNSel->SetMarkerColor(kRed);
    grNSel->Write("NSel");
    TGraph2D *grNGen =new TGraph2D(NbinM*NbinCos, xM13, yM12,  NGen);
    ...
    int binIdx=0;
    double *y0=new double[NbinM];
    double *y1=new double[NbinM];
    double *y2=new double[NbinM];
    double *sWave =new double[NbinM]; 
    double *pWave =new double[NbinM]; 
    double *y0_bkg=new double[NbinM];
    double *y1_bkg=new double[NbinM];
    double *y2_bkg=new double[NbinM];
    double *sWave_bkg =new double[NbinM]; 
    double *pWave_bkg =new double[NbinM]; 
    double *delPhiP =new double[NbinM]; 
    double *delPhiM =new double[NbinM]; 
    double *dY=new double[NbinM];
    TH1F* hSWave = new TH1F("hSWave","|S|^{2}",NbinM, hBeg, hEnd);
    TH1F* hPWave = new TH1F("hPWave","|P|^{2}",NbinM, hBeg, hEnd);
    TH1F* hY0 = new TH1F("hY0", "<Y_{0}^{0}>",NbinM, hBeg, hEnd);
    TH1F* hY1 = new TH1F("hY1", "<Y_{1}^{0}>",NbinM, hBeg, hEnd);
    TH1F* hY2 = new TH1F("hY2", "<Y_{2}^{0}>",NbinM, hBeg, hEnd);
    TH1F* hY0_bkg = new TH1F("hY0_bkg", "<Y_{0}^{0}>_bkg",NbinM, hBeg, hEnd);
    TH1F* hY1_bkg = new TH1F("hY1_bkg", "<Y_{1}^{0}>_bkg",NbinM, hBeg, hEnd);
    TH1F* hY2_bkg = new TH1F("hY2_bkg", "<Y_{2}^{0}>_bkg",NbinM, hBeg, hEnd);
    TH1F* cosHel = new TH1F("cosHel", "cosHel",NbinCos, -1, 1);
    TH1F* cosHel0 = new TH1F("cosHel0", "cosHel0",NbinCos, -1, 1);
    for(int binIdx=0; binIdx < NbinM; binIdx++){
        y0[binIdx]=0;
        y1[binIdx]=0;
        y2[binIdx]=0;
    ...
    for(int iD = 0; iD < tD->GetEntries(); iD++){
        ...
        if(binIdx < 0 || binIdx >= NbinM) continue;
        if(cosIdx < 0 || cosIdx >= NbinCos) continue;
        if(binIdx==NbinM){
            binIdx--;
        }

        if(binIdx < 0 || binIdx >= NbinM){
            cout<<"m12= "<<m12<<" mBeg= "<<mBeg<<" del= "<< m12-mBeg<<" binWidth= "<<binWidth<<" binIdx = " << binIdx<<endl;
        }
        eff = effArray[cosIdx * NbinM + binIdx];
        //eff=1;
    ...
    norm=0;
    TH1F *m12eff_bkg=new TH1F("m12eff_bkg","m12eff_bkg",NbinM, mBeg, mEnd);
    ...
    hPWave->Write(); 
    TGraph * gP = new TGraph(NbinM, xm12, pWave);
    gP->Write("pWave");
    TGraph * gS = new TGraph(NbinM, xm12, sWave);
    gS->Write("sWave");
    TGraph * gPhiP = new TGraph(NbinM, xm12, delPhiP);
    gPhiP->Write("delPhiP");
    TGraph * gPhiM = new TGraph(NbinM, xm12, delPhiM);
    gPhiM->Write("delPhiM");
    TGraph *gY0 = new TGraph(NbinM, xm12, y0);
    gY0->Draw("ap");
    gY0->Write("Y0");
    TGraph *gY1 = new TGraph(NbinM, xm12, y1);
    gY1->Draw("ap");
    gY1->Write("Y1");
    TGraph *gY2 = new TGraph(NbinM, xm12, y2);
    gY2->Draw("ap");
    gY2->Write("Y2");
    TGraph *gdY = new TGraph(NbinM, xm12, dY);
    gdY->Draw("ap");
    gdY->Write("dY");
